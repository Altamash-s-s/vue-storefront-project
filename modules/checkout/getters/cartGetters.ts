...
export const getTotals = (cart: Cart): Totals => {
  ...
  const totalTax = appliedTaxes.reduce((total, tax) => {
    return total + (tax.amount.value || 0);
  }, 0);

  // Calculate the total amount without subtracting tax from the subtotal
  const totalWithoutTax = subtotalExcludingTax;

  return {
    total: cart.prices.grand_total.value,
    subtotal: totalWithoutTax,
    totalTax: totalTax,
    special: (cart.prices.discounts) ? totalWithoutTax - calculateDiscounts(cart.prices.discounts) : totalWithoutTax,
  } as Totals;
};
